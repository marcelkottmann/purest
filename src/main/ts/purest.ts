function render(baseElement:HTMLElement, directive) {
      for (var prop in directive) {
        ...
        if (directive.hasOwnProperty(prop)) {
            ...
            var element = sel==='.'?baseElement:<HTMLElement>baseElement.querySelector(sel);
...
function renderPage(selector:string, directive) {
